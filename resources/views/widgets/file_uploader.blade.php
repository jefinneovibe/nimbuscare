<div class="col-{{@$config['style']['col_width']?:6}} inputDnD">
        <div class="form-group">
            <label class="titles" for="inputFile">{{ucfirst($config['label'])}} @if($config['id'] != 'copyOfPolicyPdf') <span>*</span> @endif
              @if(@$value['url'])
                <a class="uploaded_excl_icon" target="_blank" href="{{@$value['url']}}">
            ...
            {{-- <input type="file" id="{{@$config['id']}}" class="@if(@$value['url']) removeRequired @endif form-control-file text-primary font-weight-bold"  name="{{$config['fieldName']}}"  value="{{@$value['url']}}" placeholder="{{$config['placeHolder']}}"  onchange="readUrl(this)" data-title=" + Upload your file"> --}}

             <div class="custom-file mb-3">
                <input type="file" class="custom-file-input {{@$config['class']}} @if(@$value['url']) removeRequired @endif"  id="{{@$config['id']}}"  name="{{$config['fieldName']}}"  value="{{@$value['url']}}" placeholder="{{$config['placeHolder']}}"  onchange="readUrl(this)" data-title=" + Upload your file">
