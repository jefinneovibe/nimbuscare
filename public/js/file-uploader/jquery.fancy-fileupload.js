		...
		var AddFile = function(uploads, e, data) {
			...
			var RemoveFile = function(e) {
				...
				else
				{
                    ...
                    var delete_item = data.output_file;
                    // console.log(output_file);
                    delete_item = delete_item.replace(/\s+/g, '-');
                    console.log(delete_item);

                        if(output_file.length!=0)
                        {
                            var index = output_file.indexOf(delete_item);
                            if (index !== -1) {
                                output_file[index] = '0';
                                output_url[index] = '0';
                            }
                        ...
                        var index = output_file_name.indexOf(delete_item);
                            if (index !== -1) {
                                output_file_name[index] = '0';
                                output_url_file[index] = '0';
                            }
					}
					inforow.remove();
			...
			if (settings.accept)
			{
				...
				var found = false; var extensionArray='';
				for (var x = 0; x < settings.accept.length && !found; x++)
				{
                    extensionArray+= ', '+settings.accept[x];
					if (settings.accept[x] === fileext || settings.accept[x] === data.files[0].type)  found = true;
                }
				if (!found)  data.ff_info.errors.push(Translate('Please upload a valid file with extensions'+extensionArray+'.'));
			}
			...
			if (settings.edit && !data.ff_info.errors.length)
			{
				inforow.find('.ff_fileupload_filename').append($('<input class="form_input">').attr('type', 'text').attr('required','true').attr('name','file_upload_text[]').attr('id',makeid()).val(filename).keydown(function(e) {
			...
			if (!data.ff_info.errors.length)
			{
                ...
                inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Start uploading')).click(StartUpload).hide());
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('Start uploading')).click(StartUpload).hide());

				inforow.addClass('ff_fileupload_queued');
			}

			inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('title', 'Delete').attr('aria-label', Translate('Remove from list')).click(RemoveFile));
